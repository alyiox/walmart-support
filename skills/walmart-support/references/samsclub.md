...
`--portal samsclub`. A separate Salesforce org (`00DE0000000ZuEH`) from Walmart's
(`00D61000000ZKTc`), running the same app: same Aura transport, same `AC_*` Apex controllers, same
`Case_Category__c` schema.

Verified end to end by filing case `00019047` through this CLI — create, attach, reply — and reading
...
given (`00019047` landed as `API-AdCases` / `Product Related Questions`). The account is resolved
...
instead — which is why `00019047` came back with *Advertisers Affected: PacVue*, the account name,
...
Case numbers are zero-padded and 8 digits (`00019042`), not Walmart's bare `15957474`.
...
evidence of anything. An API case (`00019047`) carries `Issue_Category__c`, `Sub_Category_1__c`,
`Advertisers_Affected__c` and `Sponsored_Ad_Contact_{Name,Email}__c`. An onboarding case
(`00019042`) carries none of those and instead has `Account_Admin_*`, `Company_Name_Supplier__c`,
